...
Communication chain:
    Raspberry Pi 5  -->  Arduino (USB/SoftwareSerial relay)  -->  RoboClaw 2x30A
    (this script)        /dev/ttyACM0 @ 9600 baud                UART @ 38400 baud

NOTE ON BAUD MISMATCH:
    The serial port opened here uses 9600 baud because that is the rate at
    which the Arduino's SoftwareSerial link talks to the Pi.  The RoboClaw
    itself is configured at 38400 baud; the Arduino bridges the two.
    DO NOT "fix" the 9600 here to 38400 — that would break the Pi↔Arduino link.

PROOF-OF-CONCEPT NOTE:
    The Pi↔Arduino relay is a temporary, proof-of-concept arrangement.
    It is expected to be replaced later with an ESP32-S3 running dual
    hardware UARTs (one for the Pi, one for the RoboClaw) for lower
    latency and higher baud-rate headroom.
...
SERIAL_PORT   = "/dev/ttyACM0"   # Arduino's USB serial device on the Pi
BAUD_RATE     = 9600              # Pi↔Arduino baud rate (NOT the RoboClaw rate)
...
def run_test(rc, addr: int) -> None:
    ...
    print("\n" + "=" * 60)
    print("RoboClaw M1 Open-Loop Test Sequence")
    print("=" * 60)
...
def main() -> None:
    ...
    print(f"Opening serial connection to Arduino relay on {SERIAL_PORT} @ {BAUD_RATE} baud ...")
    print("  (Relay forwards to RoboClaw @ 38400 baud — do not change 9600 here)")
